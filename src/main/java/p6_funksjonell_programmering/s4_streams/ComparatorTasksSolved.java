...
public class ComparatorTasksSolved {
	...
	public static List<Person> task3(List<Person> people) {
		return people.stream().sorted(Comparator.comparing(Person::getAge)).toList();
	}
	...
	public static List<Person> task6(List<Person> people) {
		return people.stream().sorted(Comparator.comparing(Person::getName).reversed()).toList();
	}
	...
	public static List<List<String>> task8(List<List<String>> nestedLists) {
		return nestedLists.stream()
				.sorted(Comparator.comparing(l -> l.stream().mapToInt(String::length).sum()))
				.toList();
	}
	...
	public static List<Department> task9(List<Department> departments) {
		return departments.stream()
				.sorted(Comparator.comparing(d -> d.getEmployees().stream().mapToDouble(Employee::getSalary).sum()))
				.toList();
	}
